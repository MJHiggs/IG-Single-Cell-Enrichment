...
#Read in the supplementary file from GSE89497#
getGEOSuppFiles("GSE143437")

#move to the GSE directory and read in the metadata#
setwd("GSE143437")
meta <- data.frame(fread("GSE143437_DeMicheli_MuSCatlas_metadata.txt.gz"))

#Create dictionary of cell identities linked to cell barcodes#
dict <- meta$cell_annotation
names(dict) <- meta$V1

### STAGE 1 DATA PREPARATION AND UPREGULATION ANALYSIS ###################################################################################

#Read in raw and processed data and turn V1 into rownames#
processed <- data.frame(fread("GSE143437_DeMicheli_MuSCatlas_normalizeddata.txt.gz"))
rownames(processed) <- processed$V1
processed$V1 = NULL
raw <- data.frame(fread("GSE143437_DeMicheli_MuSCatlas_rawdata.txt.gz"))
rownames(raw) <- raw$V1
raw$V1 = NULL
...
filter  <- rowSums(raw != 0) >= 20
processed <- processed[filter,]

#Transform processed and convert to data.frame#
processed <- t(processed)
wil <- data.frame(processed)
...
fwrite(p, paste("Outputs/muscle_p.csv", sep=""))
colnames(percent_in) <- c("gene", iden)
fwrite(percent_in, paste("Outputs/muscle_percent_in.csv.csv", sep=""))
colnames(percent_rest) <- c("gene", iden)
fwrite(percent_rest, paste("Outputs/muscle_percent_rest.csv", sep=""))
...
#Save this file
fwrite(fc, paste("Outputs/muscle_fc.csv", sep=""))
...
fwrite(q, paste("Outputs/muscle_q.csv.csv", sep=""))
...
IG2 <- IG[IG$Gene %in%  IGs$gene[IGs$Identity == "MuSCs and progenitors"],]
...
dot <- ggplot(D2, aes(x=iden, y=gene, color=ifelse(fc == 0, NA, fc), size=ifelse(avg==0, NA, avg))) + 
  ...
  coord_flip()+
  labs(x = "Cell Lineage Identity (Skeletal Muscle - De Micheli et al., 2020)", y = "Imprinted Gene", size = "Normalised\nMean\nExpression", color = "Log2FC vs\nBackground\n(MEGs)")

pdf("Outputs/Muscle_Cell_lineage_DOTPLOT.pdf", paper= "a4r",  width = 28, height = 18)
