...
getGEOSuppFiles("GSE135132")

#read in expression matrix and cell identities#
counts <- fread("GSE135132/GSE135132_Adult_DR_GEO_counts_cpm.txt.gz")

#Read in cell metadata acquired from authors#
ID <- read.table("metadata.txt")

#create dictionary for cell identities#
dict <- ID$Named_cluster
names(dict) <- rownames(ID)

### STAGE 1 DATA PREPARATION AND UPREGULATION ANALYSIS ###################################################################################

#log transform the prescaled data ignoring genename column#
counts2 <- log(counts[,2:ncol(counts)] + 1)

#recreate genename column in counts2#
counts2$gene <- counts$V1

#select genes for only those that express in 20 cells#
percell <- (rowSums(counts2 != 0)) >= 20
counts2 <- counts2[percell,]

#transform the dataset#
wil <- data.frame(t(counts2))

#name genenames to colnames#
colnames(wil) <- counts2$gene
wil <- wil[-nrow(wil),]

#Ensure only complete cases are used#
wil <- wil[complete.cases(wil), ]

#create identity column using cell identity dict#
...
iden <- sort(unique(as.character(wil$iden)))
...
fwrite(p, paste("Outputs/Ren_p.csv", sep=""))
colnames(percent_in) <- c("gene", iden)
fwrite(percent_in, paste("Outputs/Ren_percent_in.csv", sep=""))
colnames(percent_rest) <- c("gene", iden)
fwrite(percent_rest, paste("Outputs/Ren_percent_rest.csv", sep=""))
...
#Save this file#
fwrite(fc, paste("Outputs/Ren_fc.csv", sep=""))
...
fwrite(q, paste("Outputs/Ren_q.csv", sep=""))
...
for (r in 1:length(limit)){
  ...
  ifelse(!dir.exists(file.path(paste(getwd(), "/Outputs/", sep = ""), limit[r])), dir.create(file.path(paste(getwd(), "/Outputs/", sep = ""), limit[r])), FALSE)
  ...
  fwrite(Genelist, paste("Outputs/", limit[r], "/IG_Subpopulation_List.csv", sep =""))
  fwrite(DEGs, paste("Outputs/", limit[r], "/Upregulated_Genes.csv", sep =""))
  fwrite(IGs, paste("Outputs/", limit[r], "/Upregulated_IGs.csv", sep =""))
...
### GENE SET ENRICHMENT ANALYSIS (GSEA) ########################################################################  
  ...
  if(length(tissue_GSEA) > 0){
    ifelse(!dir.exists(file.path(paste(getwd(), "/Outputs/", limit[r], sep = ""), "GSEA")), dir.create(file.path(paste(getwd(), "/Outputs/", limit[r], sep = ""), "GSEA")), FALSE)
    for(f in 1:length(tissue_GSEA)){
      ...
      pdf(paste("Outputs/", limit[r],"/GSEA/",gsub("[^A-Za-z0-9 ]","",tissue_GSEA[f]), "_GSEA.pdf", sep=""))
...
### IMPRINTED GENE TOP TISSUE EXPRESSION ######################################################################    
  ...
  fwrite(u, paste("Outputs/", limit[r], "/IG_Top_Expressed_Subpopulation.csv", sep =""))

  #write the finished Fish file#
  fwrite(Fish, paste("Outputs/", limit[r], "/Enrichment_Analysis.csv", sep =""))
}  
...
IG2 <-IG[IG$Gene %in% IGs$gene[IGs$Identity %in% c("DR-1", "DR-2", "DR-5", "DR-6")],]
...
dot <- ggplot(D2, aes(x=iden, y=gene, color=ifelse(fc == 0, NA, fc), size=ifelse(avg==0, NA, avg))) + 
  ...
  coord_flip()+
  labs(x = "5-HT Neuron Identity (Raphe Nuclei)", y = "Imprinted Gene", size = "Normalised\nMean\nExpression", color = "Log2FC vs\nBackground\n(MEGs)")

pdf("Outputs/Ren_5HT_Neuron_DOTPLOT.pdf", paper= "a4r",  width = 28, height = 18)
