...
P7_Raw <- readRDS(gzcon(url("https://github.com/pwh124/sc-da-parkinsons/blob/master/data/P7.dat.filter.final.Rds?raw=true")))
...
cluster_dict <- c(ClusterE15,ClusterP7)
...
for (e in 2:(ncol(q))){
  ...
  ig <- ORA %>% filter(sapply(strsplit(as.character(ORA$gene), "[.]"), `[`, 1) %in% IG$ï..Ensembl)
...
if(length(tissue_ORA) > 0){
  ...
  for (c in 1:nrow(Fish)){
    if(Fish$Identity[c] %in% tissue_ORA){
      ...
      test <- data.frame(gene.interest=c(as.numeric(Fish[c,3]),as.numeric(sum(q$ensmbl %in% IG$ï..Ensembl) - Fish[c,3])), gene.not.interest=c((as.numeric(Fish[c,2]) - as.numeric(Fish[c,3])),as.numeric(nrow(q) - Fish[c,2])- as.numeric(sum(q$ensmbl %in% IG$ï..Ensembl) - Fish[c,3])))
...
rest <- DEGs[!(sapply(strsplit(as.character(DEGs$gene), "[.]"), `[`, 1)  %in% IG$ï..Ensembl),] %>% group_by(Identity) %>% summarise("mean" = mean(as.numeric(fc)))
...
D <- wil[,sapply(strsplit(as.character(colnames(wil)), "[.]"), `[`, 1) %in% IG$ï..Ensembl]
...
FC <- fc %>% gather(iden, fc, -ensmbl) 
FC <- FC %>% filter(sapply(strsplit(as.character(FC$ensmbl), "[.]"), `[`, 1) %in% IG$ï..Ensembl)
...
IG2 <-IG[IG$Gene %in% IGs$gene[IGs$Identity %in% c("P7.FB.1", "P7.FB.2")],]
...
dot <- ggplot(D2, aes(x=iden, y=gene, color=ifelse(fc == 0, NA, fc), size=ifelse(avg==0, NA, avg))) + 
  ...
  coord_flip()+
  labs(x = "DA Neuron Identity (Brainwide)", y = "Imprinted Gene", size = "Normalised\nMean\nExpression", color = "Log2FC vs\nBackground\n(MEGs)")

pdf("Outputs/HOOK_DA_DOTPLOT.pdf", paper= "a4r",  width = 28, height = 18)
