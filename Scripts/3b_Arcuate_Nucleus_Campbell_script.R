...
getGEOSuppFiles("GSE93374")

#Read in the cell metadata as Arc1 and turn row 1 into column names#
Arc1 <- fread("GSE93374/GSE93374_cell_metadata.txt.gz")
colnames(Arc1)<- as.character(Arc1[1,])
Arc1 <- Arc1[-1,]

#Create a dictionary for neurons only#
dict_neurons <- Arc1$`10.clust_neurons`
names(dict_neurons)<- Arc1$`1.ID`

#Read in the pre-normalised data and adjust the rownames again#
Arc3 <- fread("GSE93374/GSE93374_Merged_all_020816_DGE.txt.gz")
rownames(Arc3) = Arc3$V1
Arc3$V1 = NULL

#Read in the filtered data and restrict cells and genes to those in the previously filtered data#
A <- data.frame(t(Arc3), stringsAsFactors = FALSE)
colnames(A) <- rownames(Arc3)
Arc2 <- fread("GSE93374/GSE93374_Merged_all_020816_BatchCorrected_LNtransformed_doubletsremoved_Data.txt.gz")
A <- A[rownames(A) %in% colnames(Arc2),]
A <- A[,colnames(A) %in% Arc2$V1]

#select genes for only those that express in 20 cells#
Filter <- (colSums(A > 0)) >= 20

#Create a size factor vector for each cell to 10000 reads#
Scale <- 10000/rowSums(A)
aa <- data.frame(t(A))
rownames(aa) <- colnames(A)
...
#Clone aa is the neuronal only identities and removed the miss cell identity group#
aa$iden <- dict_neurons[rownames(aa)]
aa <- aa[aa$iden != "miss",]

#Take original dataset, restrict it to neurons#
A2 <- A[rownames(A) %in% rownames(aa), Filter]
rm(A)

#Redo the gene filter for just neurons and apply it to aa to get just neural 20 cell genes#
Filter <- (colSums(A2 > 0)) >= 20
Filter <- c(Filter, TRUE)
just_neurons <- aa[, Filter]

#Create iden column for the dataset using dict#
dict <- sapply(strsplit(unique(Arc1$`Neuron Subclusters`),"\\."), `[`, 2) 
dict <- c(dict, "Th/Sst", "Arx/Nr5a2", "Tbx19")
names(dict) <- c(sapply(strsplit(unique(Arc1$`Neuron Subclusters`),"\\."), `[`, 1), "n03", "n07", "n27")
just_neurons$iden <- dict[just_neurons$iden]

#Create A4 the neuron dataset with the non-ARC neurons removed#
just_ARC_neurons <- just_neurons[!(just_neurons$iden %in% c("Hdc","Oxt", "Rgs16/Vip", "Rgs16/Dlx1", "Rgs16/Slc17a6", "Nr5a1/Adcyap1", "Nr5a1/Nfib", "Nr5a1/Fam19a2", "unassigned(1)", "unassigned(2)")),]

#Create the datasets to loop over#
data_names <- c("just_neurons", "just_ARC_neurons")

#Loop over the 2 dataset we are using#
for(a in 1:length(data_list)){
  #read in the current dataset as wil#
  ...
  #Create a dataset directory in the Outputs Directory#
  ...
  fwrite(p, paste("Outputs/", data_names[a], "/ARC_p.csv", sep=""))
  colnames(percent_in) <- c("gene", iden)
  fwrite(percent_in, paste("Outputs/", data_names[a], "/ARC_percent_in.csv", sep=""))
  colnames(percent_rest) <- c("gene", iden)
  fwrite(percent_rest, paste("Outputs/", data_names[a], "/ARC_percent_rest.csv", sep=""))
  ...
  #Save this file#
  fwrite(fc, paste("Outputs/", data_names[a], "/ARC_fc.csv", sep=""))
  ...
  fwrite(q, paste("Outputs/", data_names[a], "/ARC_q.csv", sep=""))
...
IG2 <-IG[IG$Gene %in% IGs$gene[IGs$Identity %in% c("Agrp/Sst", "Th/Slc6a3")],]
...
dot <- ggplot(D2, aes(x=iden, y=gene, color=ifelse(fc == 0, NA, fc), size=ifelse(avg==0, NA, avg))) + 
  ...
  theme_classic() +
  theme(axis.text.x = element_text(angle = 90, face = "italic"), axis.title.x = element_text(angle = 180), axis.title.y.right = element_text(angle = 90), axis.text.y = element_text(angle = 180, face = "italic"), legend.title.align=0.5) +
  ...
  coord_flip()+
  labs(x = "ARC Neuron Identity (Campbell et al., 2017)", y = "Imprinted Gene", size = "Normalised\nMean\nExpression", color = "Log2FC vs\nBackground\n(MEGs)")

pdf("Outputs/Arc_Neuron_DOTPLOT.pdf", paper= "a4r",  width = 28, height = 18)
