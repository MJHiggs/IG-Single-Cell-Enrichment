...
#Read in the supplementary file from GSE120410#
getGEOSuppFiles("GSE120410")

#Rename files to the standard Seurat requires#
setwd("GSE120410")
file.rename(list.files(), gsub("GSE120410_", "", list.files()))
file.rename("genes.tsv.gz", "features.tsv.gz")
setwd("../")

#Read in ID file acquired from authors and create a dictionary of cell barcodes to cell identities from manual clustering#
ID <- read.csv("Cheung_meta.csv")
dict <- ID$manual.sets
names(dict) <- ID$Barcode

### STAGE 1 DATA PREPARATION AND UPREGULATION ANALYSIS ###################################################################################

#Read data in and convert to Seurat object
counts <- Read10X(data.dir = "GSE120410")
seurat<-CreateSeuratObject(counts = counts, min.cells = 20, min.features = 200, project = "10X_PIT", assay = "RNA")

#Extract assay data into a matrix#
x <- GetAssayData(object = seurat, assay = "RNA", slot = "data")
x <- as(Class = 'matrix', object = x)

#Work out scaling factor for 10000 reads per cell#
UMI <- 10000/colSums(x)

#Transform the data, scale to 10000 reads and log transform#
x <- data.frame(t(x))
x<- x*UMI
wil <- log(x+1)

#Create iden column using dictionary of cell identities#
wil$iden <- dict[rownames(wil)]

#x <- x[x$iden %in% c("Corticotropes","Gonadotropes","Lactotropes","Melanotropes","Pituitary stem cells","Proliferating Pou1f1-cells", "Somatotropes","Thyrotropes","WBCs"),]   

#Create Unique cell identity list to loop over#
iden <- sort(unique(wil$iden))

#write table of number of cells per cell identity#
write.csv(table(wil$iden), paste("Outputs/Cell_Subtypes_Frequencies.csv", sep = ""))
...
fwrite(p, paste("Outputs/Pituit_p.csv", sep=""))
colnames(percent_in) <- c("gene", as.character(iden))
fwrite(percent_in, paste("Outputs/Pituit_percent_in.csv", sep=""))
colnames(percent_rest) <- c("gene", as.character(iden))
fwrite(percent_rest, paste("Outputs/Pituit_percent_rest.csv", sep=""))
...
#Save this file#
fwrite(fc, paste("Outputs/Pituit_fc.csv", sep=""))
...
fwrite(q, paste("Outputs/Pituit_q.csv", sep=""))

#### ORA and GSEA ANALYSIS ################################################################################################  
p <- read.csv("Outputs/Pituit_p.csv", check.names = FALSE)
q <- read.csv("Outputs/Pituit_q.csv", check.names = FALSE)
fc <- read.csv("Outputs/Pituit_fc.csv", check.names = FALSE)
percent_in <- read.csv("Outputs/Pituit_percent_in.csv", check.names = FALSE)
percent_rest <- read.csv("Outputs/Pituit_percent_rest.csv", check.names = FALSE)
...
order <- c("Somatotropes", "Lactotropes", "Thyrotropes", "Posterior pituitary", "Melanotropes", "Corticotropes","Gonadotropes","Endothelia", "Connective tissue","Proliferating Pou1f1-cells", "Pituitary stem cells", "WBCs") 

#Filter original IG list with those in the dataset#
IG2 <- IG[IG$Gene %in%  IGs$gene[IGs$Identity %in% c("Lactotropes", "Somatotropes", "Thyrotropes")],]
...
dot <- ggplot(D2, aes(x=iden, y=gene, color=ifelse(fc == 0, NA, fc), size=ifelse(avg==0, NA, avg))) + 
  ...
  coord_flip()+
  labs(x = "Pituitary Cell Type (Cheung et al., 2018)", y = "Imprinted Gene", size = "Normalised\nMean\nExpression", color = "Log2FC vs\nBackground\n(MEGs)")

pdf("Outputs/Cheung_Pituitary_DOTPLOT.pdf", paper= "a4r",  width = 28, height = 18)
