...
ifelse(!dir.exists(file.path(getwd(), "input")), dir.create(file.path(getwd(), "input")), FALSE)

#Download merged data "combined.10x.mat.rds" from google docs page - "https://drive.google.com/drive/folders/1cbwkQl1Hh70xsToNhjOdom_ru7XGA3l-"#
#Download "combined.10x.mat.rds" into input directory# 
...
#read in the Identities file recieved from personal correspondence with the authors#
ID <- fread("input/mPit_cell_cluster_information.txt")

#Create sex and cell identity dictionaries to label cells with#
dict <- ID$cluster
sex_dict <- ID$sex
names(dict) <- ID$cellID
names(sex_dict) <- ID$cellID

### STAGE 1 DATA PREPARATION AND UPREGULATION ANALYSIS ###################################################################################

#Read in 10x data from input directory#
mat.ho3 <- readRDS(file="input/combined.10x.mat.rds")

#Create Seurat Object from data with minimum gene expression of 10 cells#
s2 <- CreateSeuratObject(counts = mat.ho3, min.cells = 10)

#Create percentage mitochondrial gene for each cell#
s2[["percent.mt"]] <- PercentageFeatureSet(s2, pattern = "^mt-")

#filter cells by counts, genes and mito percentage#
...
# align by seurat3 and normalised via SCT - steps taken from the original script#
...
mPit.10x.list <- PrepSCTIntegration(object.list = mPit.10x.list, anchor.features = mPit.10x.features, verbose = FALSE)
mPit.10x.anchors <- FindIntegrationAnchors(object.list = mPit.10x.list, normalization.method = "SCT", anchor.features = mPit.10x.features, verbose = FALSE)
mPit.10x.integrated <- IntegrateData(anchorset = mPit.10x.anchors, normalization.method = "SCT", verbose = FALSE)

#Extract assay data as x, convert to transformed dataframe Wil#
x <- GetAssayData(object = mPit.10x.integrated, assay = "SCT", slot = "data")
x <- as(Class = 'matrix', object = x)
wil <- data.frame(t(x))

#Filter Wil for genes expressed in 20+ cells#
wil <- wil[,colSums(wil != 0) >= 20]

#Filter Wil again fro cells with cell identities to ascribe#
wil <- wil[rownames(wil) %in% ID$cellID,]

#Create identity column with cell type identites#
wil$iden <- as.character(dict[rownames(wil)])
  ...
  fwrite(p, paste("Outputs/Ho_pit_p.csv", sep=""))
  colnames(percent_in) <- c("gene", iden)
  fwrite(percent_in, paste("Outputs/Ho_pit_percent_in.csv", sep=""))
  colnames(percent_rest) <- c("gene", iden)
  fwrite(percent_rest, paste("Outputs/Ho_pit_percent_rest.csv", sep=""))
  ...
  #Save this file#
  fwrite(fc, paste("Outputs/Ho_pit_fc.csv", sep=""))
  ...
  fwrite(q, paste("Outputs/Ho_pit_q.csv", sep=""))
  
  ### STAGE 2 ENRICHMENT ANALYSIS ########################################################################################
  
  #read back in files if starting after#
  p <- read.csv(paste("Outputs/Ho_pit_p.csv", sep=""), check.names = FALSE)
  q <- read.csv(paste("Outputs/Ho_pit_q.csv", sep=""), check.names = FALSE)
  fc <- read.csv(paste("Outputs/Ho_pit_fc.csv", sep=""), check.names = FALSE)
  percent_in <- read.csv(paste("Outputs/Ho_pit_percent_in.csv", sep=""), check.names = FALSE)
  percent_rest <- read.csv(paste("Outputs/Ho_pit_percent_rest.csv", sep=""), check.names = FALSE)
  ...
  tissue_ORA <- Fish$Identity[Fish$IG >= 5]
  ...
  order <- c("Som", "Lac", "Mel", "Cort", "Gona", "Mult","FS", "Sox2+", "Mki67+", "MacPh") 
  
  #Filter original IG list with those in the dataset#
  IG2 <- IG[IG$Gene %in%  IGs$gene[IGs$Identity %in% c("Lac", "Som")],]   
  ...
  dot <- ggplot(D2, aes(x=iden, y=gene, color=ifelse(fc == 0, NA, fc), size=ifelse(avg==0, NA, avg))) + 
    ...
    coord_flip()+
    labs(x = "Pituitary Cell Type (Ho et al., 2020)", y = "Imprinted Gene", size = "Normalised\nMean\nExpression", color = "Log2FC vs\nBackground\n(MEGs)")
  
  pdf("Outputs/10X_Pituitary_DOTPLOT.pdf", paper= "a4r",  width = 28, height = 18)
