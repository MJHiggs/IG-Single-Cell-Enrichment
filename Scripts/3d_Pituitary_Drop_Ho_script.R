...
#Download merged data "combined.dropseq.mat.rds" from google docs page - "https://drive.google.com/drive/folders/1cbwkQl1Hh70xsToNhjOdom_ru7XGA3l-"#
#Download "combined.dropseq.mat.rds" into input directory# 
...
#Read in dropseq data from input directory#
...
#Subset data by author recommended values#
dropseq <- subset(dropseq, subset = nFeature_RNA > 100 & nCount_RNA > 300  & nFeature_RNA < 4000 & percent.mt < 10)

#Extract the assay data and convery to transformed data frame#
x <- GetAssayData(object = dropseq, assay = "RNA", slot = "data")
x <- as(Class = 'matrix', object = x)
Wil <- data.frame(t(x))

#calculate 10000 read scaling factors#
UMI <- 10000/rowSums(Wil)

#Loop through data scaling and log transforming each cell#
for(i in 1:ncol(Wil)){
  Wil[,i] = Wil[,i] * UMI
  Wil[,i] <- log(Wil[,i]+ 1)
  print(i)
...
#filter to genes expressed in 20 cells or more#
Wil <- Wil[,colSums(Wil != 0) >= 20]

#filter to cells only with ID's available#
Wil <- Wil[rownames(Wil) %in% ID$cellID,]

#Create the dataset of the 8wk old mouse cohort and remove the 13wk lactating mouse cohort#
eight <- Wil[rownames(Wil) %in% ID$cellID[ID$genotype == "8-wk old WT"],]
eight <- eight[,colSums(eight != 0) >= 20]

  #Read in the next dataset as wil#
  wil <- eight
  #Add an iden column to wil with the cell identities
  wil$iden <- as.character(dict[rownames(wil)])
  #Filter the cells based on the sex of interest (M, F or ALL)
  wil <- wil[!(sex_filter[rownames(Wil) %in% ID$cellID[ID$genotype == "8-wk old WT"]]%in% filter_value[a]),]
...
#### STAGE 3 - VISUALISATION DOTPLOT ########################################################################################################
  ...
  pdf("Outputs/Pituitary_DOTPLOT.pdf", paper= "a4r",  width = 28, height = 18)
