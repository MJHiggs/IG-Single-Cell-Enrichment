...
scale <- 10000/as.numeric(data[8,-1])

#For every row, scale to 10,000 reads and log2 normalize#
for (e in 2:nrow(d)){
  v <- as.numeric(d[e,]) * scale[e-1]
  d[e,] <- log2(v + 1)
  print(e)
} 
...
for(a in 1:length(data_names)){
  ...
  fwrite(p, paste("Outputs/", data_names[a], "/Romanov_p.csv", sep=""))
  colnames(percent_in) <- c("gene", iden)
  fwrite(percent_in, paste("Outputs/", data_names[a], "/Romanov_percent_in.csv", sep=""))
...
### IMPRINTED GENE CELL EXPRESSION ######################################################################    
  ...
  #### STAGE 3 - VISUALISATION DOTPLOT FOR NEURON UPREGULATED GENES ########################################################################################################
    ...
    dot <- ggplot(D2, aes(x=iden, y=gene, color=ifelse(fc == 0, NA, fc), size=ifelse(avg==0, NA, avg))) + 
      ...
      coord_flip()+
      labs(x = "Cell Lineage Identity (Whole Hypothalamus - Romanov et al., 2017)", y = "Imprinted Gene", size = "Normalised\nMean\nExpression", color = "Log2FC vs\nBackground\n(MEGs)")

    pdf("Outputs/Romanov_Cell_lineage_DOTPLOT.pdf", paper= "a4r",  width = 28, height = 18)
