...
data_names <- c("Adult", "All_Tissues")
...
for(a in 1:length(data_names)){
  ...
  if(substr(data_names[a], start = 1, stop = 3) != "All"){
    ...
    data_filter <- data[,ad] 
    #Create the gene filter again 50 reads or 20 cells and filter the data into viable genes#
    umi <- rowSums(data_filter)>= 50
    percell <- rowSums(data_filter != 0) >= 20
    Filter <- umi + percell >0
    ...
    #If data is needed with all cells#
  }else{
    #ASsign S and Wil to whole dataset values#
    wil <- data
    S <- sums
    
    #Loop over and dataset and scale and log normalise
    for(i in 1:(ncol(wil)-1)){
      wil[,i] <- wil[,i] * S[i]
      wil[,i] <- log(wil[,i]+1)
      print(i)
    ...
    #Create iden - a list of unique sorted identity types to use to organise analysis#
    iden <- sort(unique(dict))
    
    #Create a specific folder in Outputs to save this data type's files to, and write a file with the number of cells for each identity type# 
    ifelse(!dir.exists(file.path(paste(getwd(), "/Outputs", sep = ""), data_names[a])), dir.create(file.path(paste(getwd(), "/Outputs", sep = ""), data_names[a])), FALSE)
    
    #write table of number of cells per cell identity#
    write.csv(table(dict), paste("Outputs/", data_names[a], "/cell_Subtype_Frequencies.csv", sep = ""))
    
### ONE-SIDED WILCOXON TEST ALL ###################################################################################
