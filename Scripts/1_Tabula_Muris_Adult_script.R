...
IG2 <-IG[IG$Gene %in% IGs$gene[IGs$Identity %in% c("Brain_Non-Myeloid", "Pancreas", "Bladder")],]
...
dot <- ggplot(D2, aes(x=iden, y=gene, color=ifelse(fc == 0, NA, fc), size=ifelse(avg==0, NA, avg))) + 
  ...
  coord_flip()+
  labs(x = "Adult Organ Identity (Tabula Muris)", y = "Imprinted Gene", size = "Normalised\nMean\nExpression", color = "Log2FC vs\nBackground\n(MEGs)")

pdf("Outputs/TM_ADULT_DOTPLOT.pdf", paper= "a4r",  width = 28, height = 18)
