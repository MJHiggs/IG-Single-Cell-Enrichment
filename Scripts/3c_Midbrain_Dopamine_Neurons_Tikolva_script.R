...
#Read in the supplementary file from GSE116138#
getGEOSuppFiles("GSE116138")
untar("GSE116138/GSE116138_RAW.tar", exdir = "GSE116138/")
file.remove("GSE116138/GSE116138_RAW.tar")

#Read in cell identity file acquired from authors#
ID <- read_xlsx("Tiklova_meta_data.xlsx")
...
#loop through all individual cell files and appedn the two dataframes with the relevant information, print(a) as progress check#
for (a in 1:length(files)){
  file <- read.table(files[a])
  RPKM[,a+1] <- file$V3
  Counts[,a+1] <- file$V4
  print(a)
}
#Label RPKM column names after file names#
colnames(RPKM) <- c("gene", as.character(substr(files,1,nchar(files)-15)))

### get gene names from the ensembl IDs###
library('biomaRt')
...
#Create dictionary to label cell identities with#
dict_cells <- ID$Lineage
names(dict_cells) <- ID$SM

#create gene filter of 50 reads or 20 individual cell expression#
umi <- rowSums(Counts[,-1]) >= 50
percell <- rowSums(Counts[,-1] != 0) >= 20
...
fwrite(p, paste("Outputs/Tikolva_p.csv", sep=""))
colnames(percent_in) <- c("gene", iden)
fwrite(percent_in, paste("Outputs/Tikolva_percent_in.csv", sep=""))
colnames(percent_rest) <- c("gene", iden)
fwrite(percent_rest, paste("Outputs/Tikolva_percent_rest.csv", sep=""))
...
#Save this file#
fwrite(fc, paste("Outputs/Tikolva_fc.csv", sep=""))
...
fwrite(q, paste("Outputs/Tikolva_q.csv", sep=""))
