...
#Read in Data for embryonic cell types (data2)
data2 <- fread("GSE76381/GSE76381_MouseEmbryoMoleculeCounts.cef.txt.gz")

#Remove excess columns surrounding matrix
data2[1:6,1] <- data2[1:6,2]
data2 <- data2[c(-1:-2, -7),-2]
...
#Turn data into data.frame to remove factoring and make numeric#
Data2 <- data.frame(lapply(Data2, as.character), stringsAsFactors = FALSE)
Data2[,4:ncol(Data2)] <- data.frame(lapply(Data2[,4:ncol(Data2)], as.numeric), stringsAsFactors = FALSE)

#create gene filter for 50 reads or 20 cells expressed in#
umi <- colSums(Data2[,5:ncol(Data2)]) >= 50
percell <- colSums(Data2[,5:ncol(Data2)] != 0) >= 20
...
wil <- Data2[,5:ncol(Data2)]
wil <- wil[, filter]
wil$iden <- Data2$Cell_type
#Remove the unknown cluster#
wil <- wil[wil$iden != "mUnk",]
...
fwrite(p, paste("Outputs/Manno_p.csv", sep=""))
colnames(percent_in) <- c("gene", iden)
fwrite(percent_in, paste("Outputs/Manno_percent_in.csv", sep=""))
colnames(percent_rest) <- c("gene", iden)
fwrite(percent_rest, paste("Outputs/Manno_percent_rest.csv", sep=""))
...
#Save this file#
fwrite(fc, paste("Outputs/Manno_fc.csv", sep=""))
...
fwrite(q, paste("Outputs/Manno_q.csv", sep=""))
